...
//Get all user data (WITHOUT passwords)
...
//Get user by ID (WITHOUT password, only required fields)
...
// Add user handler (accepts password but doesn't return it)
...
export const updateUserHandler = async (req, res) => {
  const { id } = req.params;
  const { fullname, username, email, role, address, phone_number, age } = req.body;
  try {
    const [result] = await pool.query(
